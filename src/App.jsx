...
function boardTexture(board) {
  ...
  const label=monotone?"монотонный":twoTone?connected?"влажный (2 масти + коннектед)":"2 масти":"радужный"+(connected?" коннектед":"");
...
function evalHand(cards) {
  ...
  if(sf>=0) return {score:8e8+sf,name:"Стрит-флеш"};
  if(grouped[0].n===4) return {score:7e8+grouped[0].r*1e4,name:"Каре"};
  if(grouped[0].n===3&&grouped[1]?.n>=2) return {score:6e8+grouped[0].r*1e4,name:"Фулл-хаус"};
  if(flush) return {score:5e8+flush.slice(-5).reduce((a,v,i)=>a+v*Math.pow(13,i),0),name:"Флеш"};
  if(straight>=0) return {score:4e8+straight,name:"Стрит"};
  if(grouped[0].n===3) return {score:3e8+grouped[0].r*1e4,name:"Сет"};
  if(grouped[0].n===2&&grouped[1]?.n===2) return {score:2e8+grouped[0].r*1e4+grouped[1].r*100,name:"Две пары"};
  if(grouped[0].n===2) return {score:1e8+grouped[0].r*1e4,name:"Пара "+RANKS[grouped[0].r]};
  return {score:grouped[0].r*1e4,name:"Старшая "+RANKS[grouped[0].r]};
}
...
function postflopAdvice(heroCards, board, betFacing, ip=true) {
  ...
  const all=[...heroCards,...board];
  if(all.length<5) return {correct:"check", reason:"Борд не открыт"};
  ...
  if(name.startsWith("Стрит-флеш")||name.startsWith("Каре")){ tier=4; reason=name+" — максимальная рука, ставим для максимального извлечения валью"; }
  else if(name.startsWith("Фулл-хаус")||name.startsWith("Флеш")||name.startsWith("Стрит")){ tier=3; reason=name+" — очень сильная рука, бет: оппонент часто платит с худшими руками"; }
  else if(name.startsWith("Сет")||name.startsWith("Две пары")){ tier=3; reason=name+" — сильная рука + потенциал фулл-хауса, бет для валью и защиты от дро оппонента"; }
  else if(name.startsWith("Пара")){
    const pairRankName=name.split(" ")[1];
    const pairIdx=RANKS.indexOf(pairRankName);
    if(pairIdx<=4){ tier=2; reason=name+" — топ-пара с хорошим кикером, бет для валью: оппонент платит с дро и худшими парами"; }
    else if(pairIdx<=7){ tier=1; reason=name+" — средняя пара, слабовато для бета: лучшие руки коллируют, худшие фолдируют. Чек для контроля"; }
    else { tier=0; reason=name+" — слабая пара на борду с оверкартами, высокий риск быть за битым. Чек/фолд линия"; }
  } else {
    ...
    if(flushDraw){
      ...
      reason=(hasNuts?"Натс ":"")+"флеш-дро (~9 аутов, ~35% эквити) — бет для полублефа: складываем эквити дро + давление на оппонента";
    } else if(straightDraw==="oesd"){
      tier=2; reason="OESD (~8 аутов, ~32% эквити) — достаточно для полублефа: если фолдит забираем банк, если коллирует — у нас есть шансы попасть";
    } else if(straightDraw==="gutshot"){
      tier=1; reason="Гатшот (~4 аута, ~17% эквити) — недостаточно для полублефа: не хватает эквити чтобы ставить, чек и смотрим бесплатно";
    } else {
      ...
      if(bestHeroIdx===0){
        tier=1;
        reason=overcards>=2
          ? "Туз хай без пары — есть "+overcards+" оверкарты (потенциал топ-пары), но без дро нет смысла ставить. Чек — смотрим следующую карту бесплатно"
          : "Туз хай без пары и без дро — бет не получит колла от худших рук. Чек для контроля банка и защиты от рейза";
      } else {
        tier=0;
        reason=name+" без пары и без дро — нет ни валью ни полублефа, любая ставка теряет деньги. Чек/фолд линия";
      }
    }
  }
  ...
  const posCtx = ip ? "IP (в позиции)" : "OOP (вне позиции)";
  const boardCtx = texture.wet ? ` на ${texture.label} борду` : texture.paired ? " на парном борду" : "";

  if(betFacing){
    if(tier>=callThreshold){
      const r2=reason.includes("—")?reason:reason+" —";
      return {correct:"call", reason:`${name}${boardCtx} — колл для валью (${posCtx})`};
    }
    if(tier===1&&ip) return {correct:"call", reason:`${name} — слабо, но IP позволяет защититься, колл`};
    return {correct:"fold", reason:`${name}${boardCtx} — недостаточно эквити для колла (${posCtx}), фолд`};
  } else {
    if(tier>=betThreshold){
      const ipBonus=ip?" IP даёт позиционное преимущество":"";
      return {correct:"bet", reason:reason+ipBonus};
    }
    if(tier===1&&!ip&&texture.wet){
      return {correct:"check", reason:`${name}${boardCtx} — OOP на влажном борду, чек для контроля`};
    }
    if(tier===1&&ip){
      return {correct:"check", reason:`${name}${boardCtx} — недостаточно для бета IP, чек и смотрим`};
    }
    return {correct:"check", reason:reason+(texture.wet?` (${texture.label} борд — осторожнее)`:"")};
  }
}
...
function oppPostflopAction(oppCards, board, heroBet) {
  ...
  // Opp tier
  let tier=0;
  if(name.startsWith("Стрит-флеш")||name.startsWith("Каре")) tier=4;
  else if(name.startsWith("Фулл-хаус")||name.startsWith("Флеш")||name.startsWith("Стрит")) tier=3;
  else if(name.startsWith("Сет")||name.startsWith("Две пары")) tier=3;
  else if(name.startsWith("Пара")){
  ...
  return (
    <div style={{width:W,height:H,background:"linear-gradient(160deg,#fff 0%,#f5f6f8 60%,#eceef2 100%)",borderRadius:9,boxShadow:"0 8px 28px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.9)",border:"1px solid rgba(0,0,0,0.15)",display:"flex",flexDirection:"column",justifyContent:"space-between",padding:small?"4px 5px":"7px 8px",position:"relative",animation:`dealCard 0.42s cubic-bezier(0.34,1.4,0.64,1) ${delay}s both`,overflow:"hidden",flexShrink:0,userSelect:"none"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-start",gap:1}}>
        <span style={{fontSize:small?14:21,fontWeight:900,color:tc,fontFamily:"Georgia,serif",lineHeight:1}}>{card.rank}</span>
      ...
      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{width:"82%",height:"82%",background:"linear-gradient(145deg,#5a3010,#3d1f06,#5a3010)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 30px 90px rgba(0,0,0,0.85),0 0 0 2px #7a4a20"}}>
          <div style={{width:"88%",height:"88%",background:"radial-gradient(ellipse at 38% 32%,#1e7a42 0%,#125c2e 45%,#0b3d1d 100%)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"inset 0 4px 30px rgba(0,0,0,0.5)",position:"relative"}}>
            ...
            <span style={{color:"rgba(255,255,255,0.07)",fontSize:10,letterSpacing:4,fontFamily:"Georgia,serif"}}>HOLD'EM</span>
          </div>
        </div>
      </div>
        ...
        return (
          <div key={p} style={{position:"absolute",left:`${c.x}%`,top:`${c.y}%`,transform:"translate(-50%,-50%)",zIndex:3}}>
            <div style={{width:isActive?52:42,height:isActive?52:42,borderRadius:"50%",
              ...
              <span style={{fontSize:isActive?11:9,fontWeight:"bold",color:isActive?"#1a0a00":isOpp&&oppIn?"#fff":"#6b7280",fontFamily:"Georgia,serif"}}>{p}</span>
            </div>
...
function StatsPage({posStats, stats, onBack, onReset}){
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#09090f,#0f0f1a)",padding:"14px",fontFamily:"Georgia,serif"}}>
      ...
      <div style={{maxWidth:440,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
          <button onClick={onBack} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",color:"#94a3b8",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:12}}>← Назад</button>
          <div>
            <div style={{fontSize:10,color:"#374151",letterSpacing:3}}>АНАЛИТИКА</div>
            <div style={{fontSize:18,color:"#c9a84c",fontWeight:"bold",letterSpacing:2}}>СТАТИСТИКА</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:10,marginBottom:14}}>
          {[
            {l:"РУК СЫГРАНО",v:stats.hands,c:"#c9a84c"},
            {l:"ВЕРНЫХ",v:stats.correct,c:"#4ade80"},
            {l:"ОШИБОК",v:stats.wrong,c:"#f87171"},
            {l:"ТОЧНОСТЬ",v:globalAcc+"%",c:globalAcc>=70?"#4ade80":globalAcc>=50?"#fbbf24":"#f87171"},
          ].map(s=>(
        ...
        {best&&worst&&best.p!==worst.p&&(
          <div style={{display:"flex",gap:10,marginBottom:14}}>
            <div style={{flex:1,background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.25)",borderRadius:10,padding:"12px",textAlign:"center"}}>
              <div style={{fontSize:10,color:"#4a5568",letterSpacing:2,marginBottom:4}}>ЛУЧШАЯ ПОЗИЦИЯ</div>
              <div style={{fontSize:24,color:"#4ade80",fontWeight:"bold"}}>{best.p}</div>
              <div style={{fontSize:13,color:"#4ade80"}}>{best.acc}% точность</div>
              <div style={{fontSize:10,color:"#4a5568"}}>{best.t} решений</div>
            </div>
            <div style={{flex:1,background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.25)",borderRadius:10,padding:"12px",textAlign:"center"}}>
              <div style={{fontSize:10,color:"#4a5568",letterSpacing:2,marginBottom:4}}>СЛАБАЯ ПОЗИЦИЯ</div>
              <div style={{fontSize:24,color:"#f87171",fontWeight:"bold"}}>{worst.p}</div>
              <div style={{fontSize:13,color:"#f87171"}}>{worst.acc}% точность</div>
              <div style={{fontSize:10,color:"#4a5568"}}>{worst.t} решений</div>
            </div>
          </div>
        )}
        ...
        <div style={{background:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"14px",marginBottom:14}}>
          <div style={{fontSize:10,color:"#374151",letterSpacing:3,marginBottom:12}}>ПО ПОЗИЦИЯМ</div>
          {positions.map(p=>{
            const t=posStats[p].c+posStats[p].w;
              ...
              <div key={p} style={{marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontSize:13,fontWeight:"bold",color:"#c9a84c",width:30}}>{p}</span>
                    <span style={{fontSize:10,color:"#4a5568"}}>{t>0?`${posStats[p].c}/${t} верных`:"нет данных"}</span>
                  </div>
        ...
        {withAcc.length>=3&&(
          <div style={{background:"rgba(201,168,76,0.08)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
            <div style={{fontSize:10,color:"#a07830",letterSpacing:2,marginBottom:6}}>РЕКОМЕНДАЦИЯ</div>
            {worst&&worst.acc<60&&(
              <div style={{fontSize:12,color:"#c9a84c",lineHeight:1.5}}>
                Больше всего ошибок на позиции <strong>{worst.p}</strong> ({worst.acc}%). 
                {worst.p==="UTG"?" UTG — самая узкая позиция, играйте только топ-руки.":
                 worst.p==="SB"?" SB — сложная позиция, много рук идёт в фолд.":
                 " Изучите диапазон этой позиции в редакторе."}
              </div>
            )}
            {globalAcc>=80&&<div style={{fontSize:12,color:"#4ade80"}}>Отличная точность! Попробуйте усложнить диапазоны в редакторе.</div>}
          </div>
        )}
...
// Range Editor page
function RangeEditor({ranges, onSave, onBack}){
  ...
  function reset(){
    setCurrent(c=>({...c,[pos]:new Set(DEFAULT_RANGES[pos])}));
  }

  const total=current[pos].size;
  // roughly 1326 total combos, pairs=6,suited=4,offsuit=12
  const combos=[...current[pos]].reduce((acc,h)=>{
    if(h.length===2) return acc+6;
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#09090f,#0f0f1a)",padding:"14px",fontFamily:"Georgia,serif"}}>
      <style>{`
        .cell{transition:all .1s ease;cursor:pointer;border-radius:3px;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:bold;font-family:Georgia,serif;}
        .cell:active{transform:scale(0.92);}
        .pos-btn{transition:all .15s;cursor:pointer;border:none;font-family:Georgia,serif;}
      `}</style>
      <div style={{maxWidth:440,margin:"0 auto"}}>
        {/* Header */}
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          <button onClick={onBack} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",color:"#94a3b8",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:12}}>← Назад</button>
          <div>
            <div style={{fontSize:11,color:"#374151",letterSpacing:3}}>ДИАПАЗОНЫ ПРЕФЛОПА</div>
            <div style={{fontSize:16,color:"#c9a84c",fontWeight:"bold",letterSpacing:2}}>РЕДАКТОР</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:6,marginBottom:12}}>
          {["UTG","MP","CO","BTN","SB"].map(p=>(
            <button key={p} className="pos-btn" onClick={()=>setPos(p)} style={{
              flex:1,padding:"8px 0",borderRadius:8,fontSize:11,fontWeight:"bold",letterSpacing:1,
              background:pos===p?"linear-gradient(135deg,#c9a84c,#a07830)":"rgba(255,255,255,0.04)",
              border:pos===p?"1px solid #c9a84c":"1px solid rgba(255,255,255,0.08)",
              color:pos===p?"#1a0a00":"#6b7280",
            }}>{p}</button>
          ))}
        </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{fontSize:11,color:"#6b7280"}}>
            <span style={{color:"#4ade80",fontWeight:"bold"}}>{total}</span> рук · <span style={{color:"#c9a84c",fontWeight:"bold"}}>{combos}</span> комбо · <span style={{color:"#94a3b8"}}>{(combos/1326*100).toFixed(1)}%</span> диапазон
          </span>
          <button onClick={reset} style={{background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.25)",color:"#f87171",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:10}}>сброс</button>
        </div>

        {/* Legend */}
        <div style={{display:"flex",gap:10,marginBottom:8,fontSize:9,color:"#4a5568"}}>
          <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:"#166534",display:"inline-block"}}/>В диапазоне</span>
          <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.08)",display:"inline-block"}}/>Фолд</span>
          <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:"rgba(99,179,237,0.15)",display:"inline-block"}}/>Подходящие (диаг)</span>
        </div>

        {/* 13x13 grid */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(13,1fr)",gap:2,marginBottom:14}}>
          {RANKS.map((r,row)=>RANKS.map((c,col)=>{
            const hand=handAt(row,col);
            const inRange=current[pos].has(hand);
            const isPair=row===col;
            const isSuited=row<col;
            return (
              <div key={hand} className="cell" onClick={()=>toggle(hand)}
                style={{
                  aspectRatio:"1",
                  background:inRange
                    ...
                    : isPair?"rgba(99,179,237,0.08)"
                      : isSuited?"rgba(255,255,255,0.05)"
        ...
        {/* Диагональ объяснение */}
        <div style={{fontSize:10,color:"#374151",textAlign:"center",marginBottom:14,letterSpacing:1}}>
          СИНИЙ = пары · ЗЕЛЁНЫЙ = suited · КРАСНЫЙ = offsuit
        </div>

        <button onClick={()=>onSave(current)} style={{width:"100%",padding:"14px 0",borderRadius:12,background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)",border:"1px solid rgba(99,179,237,0.3)",color:"#fff",fontSize:14,fontWeight:"bold",letterSpacing:3,cursor:"pointer",fontFamily:"Georgia,serif"}}>
          СОХРАНИТЬ И ИГРАТЬ
        </button>
      </div>
    </div>
...
export default function App(){
  ...
  const [mode, setMode] = useState("full"); // "full" | "preflop"
  ...
  const deal = useCallback((currentRanges=null)=>{
    ...
    if(preflopScenario==="vs_open" && openedBy){
      initialChips={[openedBy]:{label:"РЕЙЗ",color:"#16a34a"}};
    } else if(preflopScenario==="vs_open_and_call" && openedBy && callerBeforeHero){
      initialChips={
        [openedBy]:{label:"РЕЙЗ",color:"#16a34a"},
        [callerBeforeHero]:{label:"КОЛЛ",color:"#1d4ed8"},
      };
    }
  ...
  function act(action, betSize=0.5){
    ...
    if(ng.street==="preflop"){
      ...
      if(ng.facingThreeBet){
        ...
        if(FOURBET.has(ng.hand)){
          correctAction="raise";
          note=`${ng.hand} — 4-бет для валью против 3-бета`;
        } else if(CALL3BET.has(ng.hand)){
          correctAction="call";
          note=`${ng.hand} — колл против 3-бета, достаточно эквити`;
        } else {
          correctAction="fold";
          note=`${ng.hand} — фолд против 3-бета, недостаточно эквити`;
        }
        ...
        // Call or 4-bet
        ng.log.push(action==="raise"?`Вы 4-бетите`:`Вы коллируете 3-бет`);
        ng.facingThreeBet=false;
        ng.betFacing=false;
        ng.chips={[ng.pos]:{label:action==="raise"?"4-БЕТ":"КОЛЛ",color:action==="raise"?"#dc2626":"#1d4ed8"}};
      ...
      if(ng.preflopScenario==="vs_open"&&!ng.scenarioDone){
        ...
        if(myThreebetRange.has(ng.hand)){
          correctAction="raise"; note=`${ng.hand} — 3-бет против открытия ${openPos}`;
        } else if(myColdcallRange.has(ng.hand)){
          correctAction="call"; note=`${ng.hand} — колл против открытия ${openPos}`;
        } else {
          correctAction="fold"; note=`${ng.hand} — фолд, недостаточно против открытия ${openPos}`;
        }
        const playerAction=action==="raise"?"raise":action==="call"?"call":"fold";
        const correct=playerAction===correctAction;
        ng.decisions.push({street:`Префлоп vs ${openPos}`,action:action.toUpperCase(),correct,note});
        setStats(s=>({...s,correct:s.correct+(correct?1:0),wrong:s.wrong+(correct?0:1)}));
        setPosStats(ps=>({...ps,[ng.pos]:{c:ps[ng.pos].c+(correct?1:0),w:ps[ng.pos].w+(correct?0:1)}}));
        ng.scenarioDone=true;
        ng.result="fold_preflop";
        ng.chips={[ng.pos]:{label:action==="raise"?"3-БЕТ":action==="call"?"КОЛЛ":"ФОЛД",
      ...
      if(ng.preflopScenario==="vs_open_and_call"&&!ng.scenarioDone){
        ...
        if(SQUEEZE.has(ng.hand)){
          correctAction="raise"; note=`${ng.hand} — сквиз против ${openPos}+${callPos}`;
        } else {
          correctAction="fold"; note=`${ng.hand} — фолд, coldcall в мультипот невыгоден`;
        }
        const playerAction=action==="raise"?"raise":action==="call"?"call":"fold";
        const isCorrect=playerAction===correctAction||(playerAction==="fold"&&correctAction==="fold");
        const correct=playerAction===correctAction;
        ng.decisions.push({street:`Сквиз vs ${openPos}+${callPos}`,action:action.toUpperCase(),correct,note});
        setStats(s=>({...s,correct:s.correct+(correct?1:0),wrong:s.wrong+(correct?0:1)}));
        setPosStats(ps=>({...ps,[ng.pos]:{c:ps[ng.pos].c+(correct?1:0),w:ps[ng.pos].w+(correct?0:1)}}));
        ng.scenarioDone=true;
        ng.result="fold_preflop";
        ng.chips={[ng.pos]:{label:action==="raise"?"СКВИЗ":action==="call"?"КОЛЛ":"ФОЛД",
          color:action==="raise"?"#dc2626":action==="call"?"#1d4ed8":"#374151"}};
        ng.chipKey=(ng.chipKey||0)+1;
        setG(ng); return;
      }
      ...
      const note=ng.inRange
        ?(correct?"✓ Рейз — "+ng.hand+" в диапазоне "+ng.pos:"✗ Надо рейзить — "+ng.hand+" в диапазоне "+ng.pos)
        :(correct?"✓ Фолд — "+ng.hand+" вне диапазона "+ng.pos:"✗ Надо фолдить — "+ng.hand+" не входит в диапазон "+ng.pos);
      ng.decisions.push({street:"Префлоп",action:action==="raise"?"РЕЙЗ":"ФОЛД",correct,note});
      setStats(s=>({...s,correct:s.correct+(correct?1:0),wrong:s.wrong+(correct?0:1)}));
      setPosStats(ps=>({...ps,[ng.pos]:{c:ps[ng.pos].c+(correct?1:0),w:ps[ng.pos].w+(correct?0:1)}}));
      ...
      if(modeRef.current==="preflop"){
        ...
        if(Math.random()<threebetChance){
          ...
          ng.log.push(`${threebetPos} 3-бетит`);
          ng.chips={[ng.pos]:{label:"РЕЙЗ",color:"#16a34a"},[threebetPos]:{label:"3-БЕТ",color:"#dc2626"}};
          ng.chipKey=(ng.chipKey||0)+1;
        ...
        ng.chips={[ng.pos]:{label:"РЕЙЗ",color:"#16a34a"}};
        ng.chipKey=(ng.chipKey||0)+1;
        setG(ng); return;
      }
      ...
      const chips={[ng.pos]:{label:"РЕЙЗ",color:"#16a34a"}};
      let anyCall=false;
      const newOpps=(ng.opps||[]).map(opp=>{
        ...
        if(threebet){
          anyCall=true;
          chips[opp.pos]={label:"3-БЕТ",color:"#dc2626"};
          ng.log.push(`${opp.pos} 3-бетит`);
          ng.betFacing=true; // hero now faces a 3-bet
        } else if(calls){
          anyCall=true;
          chips[opp.pos]={label:"КОЛЛ",color:"#1d4ed8"};
          ng.log.push(`${opp.pos} коллирует`);
        } else {
          ng.log.push(`${opp.pos} фолдит`);
        }
      ...
      if(!anyCall && newOpps.length>0){
        ...
        chips[newOpps[0].pos]={label:"КОЛЛ",color:"#1d4ed8"};
        ng.log.push(`${newOpps[0].pos} коллирует`);
      }
      ng.opps=newOpps;
    ...
    const streetLabel=ng.street==="flop"?"Флоп":ng.street==="turn"?"Терн":"Ривер";
    const betLabel=action==="bet"||action==="raise"?` (${Math.round(betSize*100)}% банка)`:""; ng.decisions.push({street:streetLabel,action:action.toUpperCase()+betLabel,correct,note:reason});
    setStats(s=>({...s,correct:s.correct+(correct?1:0),wrong:s.wrong+(correct?0:1)}));
    setPosStats(ps=>({...ps,[ng.pos]:{c:ps[ng.pos].c+(correct?1:0),w:ps[ng.pos].w+(correct?0:1)}}));

    const heroChipLabel = action==="bet"||action==="raise" ? `${Math.round(betSize*100)}%` : action==="call"?"КОЛЛ":"ЧЕК";
    ...
    const updatedOpps=(ng.opps||[]).map(opp=>{
      ...
      const lbl=act==="raise"?"РЕЙЗ":act==="call"?"КОЛЛ":act==="bet"?"БЕТ":"ЧЕК";
      ...
      allFolded=false;
      if(act==="raise") ng.log.push(`${opp.pos} рейзит`);
      else if(act==="call") ng.log.push(`${opp.pos} коллирует`);
      else if(act==="bet") ng.log.push(`${opp.pos} ставит бет`);
      else ng.log.push(`${opp.pos} чекирует`);
    ...
    if(firstOop){
      ...
      if(ng.betFacing) ng.log.push(`${firstOop.pos} ставит бет на ${next}`);
    } else {
      ng.betFacing=false;
    }
    ng.chips={};
  ...
  function showHint(){
    ...
    if(g.street==="preflop"){
      if(g.facingThreeBet){
        const FOURBET=new Set(["AA","KK","QQ","AKs","AKo"]);
        const CALL3BET=new Set(["JJ","TT","99","AQs","AJs","KQs","QJs","AQo"]);
        const advice=FOURBET.has(g.hand)?"4-БЕТ":CALL3BET.has(g.hand)?"КОЛЛ":"ФОЛД";
        setHint(`GTO vs 3-бет: ${advice} с ${g.hand}`);
        ...
        const advice=t.has(g.hand)?`3-БЕТ`:c.has(g.hand)?`КОЛЛ`:`ФОЛД`;
        setHint(`GTO vs ${openPos}: ${advice} с ${g.hand}`);
      } else if(g.preflopScenario==="vs_open_and_call"&&!g.scenarioDone){
        const SQUEEZE=new Set(["AA","KK","QQ","JJ","TT","AKs","AKo","AQs","AJs","A5s","A4s","KQs"]);
        const advice=SQUEEZE.has(g.hand)?"СКВИЗ":"ФОЛД";
        setHint(`GTO сквиз: ${advice} с ${g.hand}`);
      } else {
        setHint(g.inRange?`GTO: РЕЙЗ — ${g.hand} в диапазоне открытия ${g.pos}`:`GTO: ФОЛД — ${g.hand} вне диапазона ${g.pos}`);
      }
    } else {
  ...
  if(page==="ranges") return <RangeEditor ranges={ranges} onSave={saveRanges} onBack={()=>setPage("train")}/>
  ...
  const done=!!g.result;
  const streetLabel={"preflop":"ПРЕФЛОП","flop":"ФЛОП","turn":"ТЕРН","river":"РИВЕР"}[g.street]||"ВСКРЫТИЕ";
  const mistakes=g.decisions.filter(d=>!d.correct).length;

  const OUTCOME={
    fold_preflop:{t:g.decisions[0]?.correct?"✓ Верный фолд":"✗ Надо было рейзить",c:g.decisions[0]?.correct?"#4ade80":"#f87171"},
    win_postflop:{t:"BB сфолдил",c:"#4ade80"},
    fold_postflop:{t:"Вы сфолдировали",c:"#94a3b8"},
    win:{t:"Победа на вскрытии 🏆",c:"#4ade80"},
    lose:{t:"Проигрыш на вскрытии",c:"#f87171"},
    tie:{t:"Ничья",c:"#c9a84c"},
  };

  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#09090f 0%,#0f0f1a 50%,#080f09 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",padding:"14px 14px 30px",fontFamily:"Georgia,serif"}}>
    <style>{`
    ...
    <div style={{width:"100%",maxWidth:440}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        <div>
          <div style={{fontSize:10,letterSpacing:4,color:"#374151"}}>6-MAX NL HOLD'EM</div>
          <div style={{fontSize:18,color:"#c9a84c",fontWeight:"bold",letterSpacing:2}}>♠ RANGE TRAINER ♦</div>
        </div>
        <div style={{display:"flex",gap:6}}>
          ...
          <button className="btn" onClick={()=>setPage("ranges")} style={{background:"rgba(201,168,76,0.08)",border:"1px solid rgba(201,168,76,0.25)",color:"#c9a84c",borderRadius:8,padding:"8px 10px",fontSize:11,letterSpacing:1}}>
            📊 ДИПАЗ
          </button>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:10}}>
        {[["full","🃏 ПОЛНАЯ РУКА"],["preflop","⚡ ТОЛЬКО ПРЕФЛОП"]].map(([m,label])=>(
          <button key={m} className="btn" onClick={()=>{setMode(m);deal();}} style={{
            flex:1,padding:"8px 0",borderRadius:9,fontSize:11,fontWeight:"bold",letterSpacing:0.5,
            background:mode===m?"linear-gradient(135deg,#c9a84c,#a07830)":"rgba(255,255,255,0.04)",
            border:mode===m?"1px solid #c9a84c":"1px solid rgba(255,255,255,0.08)",
            color:mode===m?"#1a0a00":"#6b7280",
          }}>{label}</button>
        ))}
      </div>

      {/* Stats */}
      <div style={{display:"flex",justifyContent:"space-around",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"8px",marginBottom:10}}>
        {[{l:"РУКА",v:stats.hands+1,c:"#c9a84c"},{l:"ВЕРНО",v:stats.correct,c:"#4ade80"},{l:"ОШИБКИ",v:stats.wrong,c:"#f87171"},{l:"УЛИЦА",v:streetLabel,c:"#94a3b8"}].map(s=>(
          <div key={s.l} style={{textAlign:"center"}}>
            <div style={{fontSize:s.l==="УЛИЦА"?10:18,fontWeight:"bold",color:s.c,lineHeight:1}}>{s.v}</div>
      ...
      {/* Table */}
      <div style={{background:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:16,padding:"10px 10px 2px",marginBottom:10}}>
        ...
        <div style={{textAlign:"center",paddingBottom:6,fontSize:11,color:"#6b7280",letterSpacing:2}}>
          ПОЗИЦИЯ: <span style={{color:"#c9a84c",fontWeight:"bold"}}>{g.pos}</span>
          ...
          {g.betFacing&&g.street==="preflop"&&!done&&<span style={{color:"#dc2626",marginLeft:8,fontWeight:"bold"}}>· 3-БЕТ!</span>}{g.betFacing&&g.street!=="preflop"&&!done&&<span style={{color:"#fb923c",marginLeft:8}}>· ставит бет</span>}
        </div>
      ...
      {/* Board */}
      {g.board.length>0&&(
        <div key={"b"+animKey} style={{display:"flex",justifyContent:"center",gap:7,marginBottom:10}}>
        ...
        <div style={{textAlign:"center",marginBottom:8,padding:"7px 14px",borderRadius:10,
          ...
          {g.preflopScenario==="vs_open"&&(
            <span style={{fontSize:12,color:"#fca5a5"}}>
              <span style={{color:"#f87171",fontWeight:"bold"}}>{g.openedBy}</span> открыл — твой ход
            </span>
          )}
          {g.preflopScenario==="vs_open_and_call"&&(
            <span style={{fontSize:12,color:"#fca5a5"}}>
              <span style={{color:"#f87171",fontWeight:"bold"}}>{g.openedBy}</span> открыл,{" "}
              <span style={{color:"#f87171",fontWeight:"bold"}}>{g.callerBeforeHero}</span> сколлировал — сквиз?
            </span>
          )}
        </div>
      )}
      ...
      <div style={{textAlign:"center",marginBottom:10}}>
        <span style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:20,padding:"3px 18px",fontSize:12,color:"#94a3b8",letterSpacing:3}}>
          {g.hand}{g.heroEval&&<span style={{marginLeft:8,color:"#c9a84c"}}>— {g.heroEval.name}</span>}
        </span>
      </div>
      ...
      {/* Hint */}
      {hint&&<div style={{background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:10,padding:"8px 14px",marginBottom:10,fontSize:12,color:"#c9a84c",textAlign:"center",animation:"fadeIn .2s ease both"}}>{hint}</div>}
        ...
        <div>
          {g.street==="preflop"?(
            <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
              {g.facingThreeBet?(
                <>
                  <button className="btn" onClick={()=>act("raise")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#7f1d1d,#b91c1c)",border:"1px solid #ef4444",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>4-БЕТ</button>
                  <button className="btn" onClick={()=>act("call")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)",border:"1px solid #60a5fa",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>КОЛЛ</button>
                  <button className="btn" onClick={()=>act("fold")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#374151,#1f2937)",border:"1px solid #6b7280",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>ФОЛД</button>
                </>
              ):g.preflopScenario==="vs_open"&&!g.scenarioDone?(
                <>
                  <button className="btn" onClick={()=>act("raise")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#7f1d1d,#b91c1c)",border:"1px solid #ef4444",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>3-БЕТ</button>
                  <button className="btn" onClick={()=>act("call")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)",border:"1px solid #60a5fa",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>КОЛЛ</button>
                  <button className="btn" onClick={()=>act("fold")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#374151,#1f2937)",border:"1px solid #6b7280",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>ФОЛД</button>
                </>
              ):g.preflopScenario==="vs_open_and_call"&&!g.scenarioDone?(
                <>
                  <button className="btn" onClick={()=>act("raise")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#7f1d1d,#b91c1c)",border:"1px solid #ef4444",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>СКВИЗ</button>
                  <button className="btn" onClick={()=>act("call")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)",border:"1px solid #60a5fa",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>КОЛЛ</button>
                  <button className="btn" onClick={()=>act("fold")} style={{flex:1,padding:"13px 0",borderRadius:10,background:"linear-gradient(135deg,#374151,#1f2937)",border:"1px solid #6b7280",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>ФОЛД</button>
                </>
              ):(
                <>
                  <button className="btn" onClick={()=>act("raise")} style={{flex:1,padding:"14px 0",borderRadius:10,background:"linear-gradient(135deg,#14532d,#15803d)",border:"1px solid #22c55e",color:"#fff",fontSize:14,fontWeight:"bold",letterSpacing:2,boxShadow:"0 4px 20px rgba(34,197,94,0.2)"}}>↑ РЕЙЗ</button>
                  <button className="btn" onClick={()=>act("fold")} style={{flex:1,padding:"14px 0",borderRadius:10,background:"linear-gradient(135deg,#7f1d1d,#b91c1c)",border:"1px solid #ef4444",color:"#fff",fontSize:14,fontWeight:"bold",letterSpacing:2,boxShadow:"0 4px 20px rgba(239,68,68,0.2)"}}>✕ ФОЛД</button>
                </>
              )}
            </div>
          ):(
            <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
              {!g.betFacing&&(
                showBetSizes ? (
                  ...
                  <>
                    <button className="btn" onClick={()=>setShowBetSizes(true)} style={{flex:1,minWidth:80,padding:"12px 0",borderRadius:10,background:"linear-gradient(135deg,#14532d,#15803d)",border:"1px solid #22c55e",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>РЕЙЗ ▾</button>
              ...
              {g.betFacing&&(
                showBetSizes ? (
                  ...
                  <button className="btn" onClick={()=>setShowBetSizes(true)} style={{flex:1,minWidth:80,padding:"12px 0",borderRadius:10,background:"linear-gradient(135deg,#1e3a5f,#1d4ed8)",border:"1px solid #60a5fa",color:"#fff",fontSize:13,fontWeight:"bold",letterSpacing:1}}>РЕЙЗ ▾</button>
                )
              )}
          ...
          <button className="btn" onClick={showHint} style={{width:"100%",padding:"9px 0",borderRadius:10,background:"rgba(201,168,76,0.07)",border:"1px solid rgba(201,168,76,0.22)",color:"#c9a84c",fontSize:12,letterSpacing:2}}>
            💡 ПОДСКАЗКА GTO
          </button>
        </div>
      )}
      ...
      {done&&(
        <div style={{
          ...
          <div style={{
            background:"linear-gradient(180deg,#0f141a 0%,#0a0f12 100%)",
            ...
            <div style={{padding:"14px",borderRadius:12,marginBottom:10,textAlign:"center",
              ...
              <div style={{fontSize:11,color:"#4a5568",letterSpacing:3,marginBottom:4}}>GTO ОЦЕНКА</div>
              <div style={{fontSize:26,fontWeight:"bold",marginBottom:2,
                color:mistakes===0?"#4ade80":mistakes<g.decisions.length?"#fbbf24":"#f87171"}}>
                {mistakes===0?"✓ ВСЕ ВЕРНО":g.decisions.length===1?"✗ НЕВЕРНО":`${g.decisions.length-mistakes}/${g.decisions.length} верных`}
              </div>
              <div style={{fontSize:11,color:"#6b7280"}}>{mistakes===0?"Отличная игра":`${mistakes} ${mistakes===1?"ошибка":"ошибок"}`}</div>
            </div>
            ...
            {g.result&&OUTCOME[g.result]&&(
              <div style={{padding:"10px",borderRadius:10,marginBottom:10,textAlign:"center",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)"}}>
                <div style={{fontSize:10,color:"#374151",letterSpacing:3,marginBottom:3}}>ИСХОД</div>
            ...
            {g.decisions.length>0&&(
              <div style={{background:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"10px 14px",marginBottom:14}}>
                <div style={{fontSize:10,letterSpacing:3,color:"#374151",marginBottom:8}}>РАЗБОР</div>
                {g.decisions.map((d,i)=>(
